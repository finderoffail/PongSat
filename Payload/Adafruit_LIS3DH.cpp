 ...
 */
bool Adafruit_LIS3DH::begin(uint8_t i2caddr, uint8_t nWAI, int8_t tempOffset) {
  _i2caddr = i2caddr;
  _wai = nWAI;
  ...
  _tmp_cfg.write(0xC0); // RHW: enable adcs and temperature sensor. used to be: _tmp_cfg.write(0x80); // enable adcs
...
// RHW added this function
int16_t Adafruit_LIS3DH::readTemperature(void) {
  return ((readADC(3) >> 8) + _tempOffset);
}
